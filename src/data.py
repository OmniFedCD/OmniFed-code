...
class SliceWindowDataset():
    ...
    def _to_windows(self, values):
        ...
        while i < values.shape[0]:
            if i + self._window_size <= self._values.shape[0]:
                slice_windows.append(self._values[i:i+self._window_size])
            else:
                break
            i += self._window_size
            
        return np.array(slice_windows)
